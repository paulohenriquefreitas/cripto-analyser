...
class CryptoPriceSyncServiceTest {
    ...
    void shouldPersistOnlyMissingCandlesWhenFallbackFetchContainsDuplicates() {
        ...
        ArgumentCaptor<CryptoPrice> savedPricesCaptor = ArgumentCaptor.forClass(CryptoPrice.class);
        verify(cryptoPriceRepository, times(requestedCandles)).save(savedPricesCaptor.capture());
        assertThat(savedPricesCaptor.getAllValues())
                .extracting(CryptoPrice::getTimestamp)
                .containsAll(missingPrices.stream().map(CryptoPrice::getTimestamp).toList());
    }
    ...
    @Test
    void shouldIgnoreCandlesReturnedOutsideRequestedClosedWindow() {
        String symbol = "ETHUSDT";
        String interval = "1m";
        int requestedCandles = 3;
        Instant latestClosedOpenTime = CandleIntervalHelper.resolveLatestClosedOpenTime(interval);
        Instant targetStartTime = CandleIntervalHelper.resolveInitialOpenTime(interval, requestedCandles);
        List<CryptoPrice> validPrices = pricesBetween(symbol, interval, targetStartTime, requestedCandles);
        CryptoPrice currentOpenPrice = priceAt(
                symbol,
                interval,
                CandleIntervalHelper.resolveNextOpenTime(latestClosedOpenTime, interval));
        List<CryptoPrice> brokerPrices = java.util.stream.Stream
                .concat(validPrices.stream(), java.util.stream.Stream.of(currentOpenPrice))
                .toList();
        ...
        when(loxBrokerClient.getCandles(
                ...
                eq(requestedCandles),
                eq((String) null)))
                .thenReturn(toCandles(brokerPrices));
        when(cryptoPriceRepository.findBySymbolAndIntervalAndTimestampBetween(
                symbol,
                interval,
                validPrices.getFirst().getTimestamp(),
                validPrices.getLast().getTimestamp()))
                .thenReturn(List.of());
        when(cryptoPriceRepository.save(any(CryptoPrice.class)))
                .thenAnswer(invocation -> invocation.getArgument(0));
        when(cryptoPriceRepository.findFirstBySymbolAndIntervalOrderByTimestampDesc(symbol, interval))
                .thenReturn(Optional.of(validPrices.getLast()));

        var result = cryptoPriceSyncService.sync(symbol, interval, requestedCandles);

        assertThat(result.getFetchedRecords()).isEqualTo(requestedCandles);
        assertThat(result.getPersistedRecords()).isEqualTo(requestedCandles);

        ArgumentCaptor<CryptoPrice> savedPricesCaptor = ArgumentCaptor.forClass(CryptoPrice.class);
        verify(cryptoPriceRepository, times(requestedCandles)).save(savedPricesCaptor.capture());
        assertThat(savedPricesCaptor.getAllValues())
                .extracting(CryptoPrice::getTimestamp)
                .doesNotContain(currentOpenPrice.getTimestamp());
    }
